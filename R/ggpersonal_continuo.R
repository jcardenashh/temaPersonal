...
ggpersonal_continuo <- function(grafico){
  ...
  grafico+
    ggplot2::scale_y_continuous(expand  =  expand_scale ( mult  =  c(0,0.1)),labels = scales::label_comma())+
    ggplot2::scale_x_continuous(expand  =  expand_scale ( mult  =  c(0,0.1)),labels = scales::label_comma()) +
